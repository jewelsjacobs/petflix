...
const BASE_URL = 'https://api.minimaxi.chat'; // Updated Base URL
const CREATE_TASK_ENDPOINT = '/v1/video_generation'; // Updated Create Task path
const QUERY_STATUS_ENDPOINT = '/v1/query/video_generation'; // Updated Query Status path
const RETRIEVE_URL_ENDPOINT = '/v1/files/retrieve';
const API_MODEL = 'T2V-01'; // Updated model to match Python example
...
export const generateVideo = async ({ 
  ...
  /* // Temporarily commented out for testing with free tier
  try {
  ...
  */ // End temporary comment
  ...
  try {
    ...
    if (retrieveResult.base_resp?.status_code !== 0 || !retrieveResult.file?.download_url) {
        console.error("Retrieve URL API returned non-zero status or missing URL:", retrieveResult.base_resp, retrieveResult.file);
        throw new Error(ERROR_MESSAGES.API_REQUEST_FAILED);
    }

    finalVideoUrl = retrieveResult.file.download_url;
