...
generates short cinematic episodes (60-90 seconds) starring their pet.
Scene images are generated ON-DEVICE using Apple's Image Playground framework
(ImageCreator API, Animation style). The pet's appearance is described using
a text prompt generated by the Apple Foundation Models framework (iOS 26+),
which analyzes the pet's photo and produces an ImageCreator-friendly description
using proper breed terminology. This text description is passed as a `.text()`
concept to ImageCreator alongside scene descriptions.

NOTE: `ImagePlaygroundOptions.Personalization` is people-only and does NOT
work for pets. Do not use it. Foundation Models pet descriptions scored 5/5
on Recognition in testing. See PET_DESCRIPTION_RESEARCH.md for details.
These images are then assembled into video using AVFoundation with Ken Burns
effects, voiceover narration, music, transitions, and text overlays.
No server-side rendering needed. No API costs.
...
Episodes use Apple's on-device Image Playground framework to generate
scene images starring the user's pet, then AVFoundation to assemble
those images into a playable video. No server-side rendering needed.

### Layer 1: Scene Image Generation (on-device, per-episode)
- When a user taps 'Create Episode', the app uses Apple's `ImageCreator`
  API (Image Playground framework, iOS 18.4+) to generate 8-10 dramatic
  scene images for the episode
- Each scene uses a `.text()` concept containing a pet description
  (generated by Foundation Models from the pet's photo) combined with
  a dramatic scene description. No `.image()` concept needed.
- Pet description pipeline (PRIMARY — iOS 26+):
  1. Pass pet photo to Apple Foundation Models on-device LLM
  2. LLM returns structured PetDescription (breed, colors, markings,
     coat texture, facial features) using proper breed terminology
  3. Insert description into `.text()` prompt with scene description
  This is the equivalent of what Gemini did manually in testing, but
  automated, on-device, free, and offline.
- Pet description pipeline (FALLBACK — iOS 17-25):
  Core ML breed classifier + VNRecognizeAnimalsRequest (species) +
  VNDetectAnimalBodyPoseRequest (pose) + CIAreaAverage (colors).
...
- IMPORTANT: `ImagePlaygroundOptions.Personalization` is people-only
  and does NOT work for pets. Do not use it.
- Style: `.animation` ONLY — no ChatGPT styles, everything stays on-device
- Cost: $0 per image. Fully on-device. Works offline. Unlimited generations.
- No pre-generated template images needed — scenes are generated per-episode
  from text prompts tailored to each series
- Voiceover scripts, music tracks, and text overlays are pre-authored per series
- API reference: https://developer.apple.com/documentation/imageplayground/imagecreator

**Fallback (non-Apple-Intelligence devices):**
- Vision framework subject lifting (VNGenerateForegroundInstanceMaskRequest)
  + Core Image compositing (CIBlendWithMask) onto pre-generated background
  templates. Tested and working — see PET_TRANSFER_TEST_PLAN.md Phase 1.

### Layer 2: Video Assembly (on-device, AVFoundation)
- The generated scene images are composited into a playable video ON THE DEVICE
...
### Cost per episode
- ImageCreator scene generation: $0 (on-device, unlimited)
- On-device video assembly: $0
- Total: $0 per episode for ALL tiers
...
- Do NOT use ChatGPT-powered Image Playground styles — Animation style only
...
- Episodes use ImageCreator (Image Playground framework) with text-only
  prompts (pet description + scene description) via on-device AVFoundation
- Pet description generated by Apple Foundation Models framework (iOS 26+)
  or Core ML breed classifier fallback (iOS 17+)
- NOTE: Personalization API is PEOPLE-ONLY, does NOT work for pets
- Foundation Models pet descriptions scored 5/5 Recognition in testing
...
- Pet identity transfer uses Animation style via ImageCreator — on-device, $0
- Fallback for non-Apple-Intelligence devices: Vision + Core Image compositing
